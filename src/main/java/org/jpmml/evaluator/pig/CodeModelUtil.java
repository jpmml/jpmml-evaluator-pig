...
public class CodeModelUtil {
	...
	public void build(String className, File pmmlFile, File udfJarFile) throws Exception {
		...
		JClass evaluatorFuncClass = codeModel.ref(EvaluatorFunc.class);

		JDefinedClass definedEvaluatorFuncClass = codeModel._class(JMod.PUBLIC, className, ClassType.CLASS);
		definedEvaluatorFuncClass._extends(evaluatorFuncClass);
		...
		JMethod defaultConstructor = definedEvaluatorFuncClass.constructor(JMod.PUBLIC);
		...
		servicePackage.addResourceFile(new JServiceConfigurationFile(evaluatorFuncClass, Collections.<JClass>singletonList(definedEvaluatorFuncClass)));
